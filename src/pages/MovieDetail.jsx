    ...
    <main>
      <section className="flex justify-around flex-wrap py-5">
        <div className="max-w-sm mx-1">
          <img className="rounded" src={image} alt="image is missed" />
        </div>
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white mx-1">
          <h1 className="text-4xl font-bold my-3 text-center lg:text-left">{movie.title}</h1>
          <p className="my-4">{movie.overview}</p>
          {movie.genres && (
            <p className="my-7 flex flex-wrap gap-2">
              {movie.genres.map((item) =>
                (<span className="mr-2 border border-gray-200 rounded dark:border-gray-600 p-2" key={item.id}>{item.name}</span>)
              )}
            </p>)}
          <div className="flex items-center">
            ...
            <p className="ms-2 font-bold text-gray-900 dark:text-white">{movie.vote_average}/10</p>
